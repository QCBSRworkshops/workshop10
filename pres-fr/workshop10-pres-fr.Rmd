...
Quels sont les .alert[espèces indicatrices] pour chaque groupe de sites?
...
Tous les sites ont été correctement classifiés dans leur groupe de latitude en fonction des variables environnementales (*Proportion de classification correcte = 1*).
...
Essayons de .alert[prédire la classification] de 5 nouveaux sites à l'aide de notre LDA:
.small[
```{r}
...
.small[Créez 4 groupes de latitude avec des étendues égales à partir des données `mite.xy`. Ensuite, faites une LDA sur les données environnementales `mite.env` des acariens (`SubsDens` et `WatrCont`).
...
**Étape 1:** Créer 4 groupes de latitude avec des étendues égales.
