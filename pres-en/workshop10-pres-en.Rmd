...
1. Contrast and recognize the differences between unconstrained and constrained ordination analyses in ecological data;

2. Learn how to perform and interpret the outcomes of:
...
Using constrained ordination methods, we will also learn:
...
These analyses allow us to **describe** and **predict** relationships between community composition data and environmental variables.

<br>

.center[_This means we can **test hypotheses** related to species responses to selection gradients!_]
...
We must remove sites without species, to avoid biased association measures.

.pull-left[
We could do this using this:
...
But, more appropriately, we can keep the lines (sites) with values different than zero.
...
summary(spe) # summary statistics for all objects (e.g., min, mean, max)
```
...
```
.alert[Notice]: There are many zeros!

---
# Distribution of species frequencies

How many zeroes are in the dataset?
```{r}
sum(spe == 0)
```
<br>
...
.alert[**Over 50%**] of our dataset consists of zeroes, which is common in community data sets.

--

However, we do not want these common absences to **artificially increase** the similarity between sites.

To avoid this, we can .alert[transform] the community data using the .alert[Hellinger] transformation:
```{r}
...
> **Hellinger's transformation** allows us to reduce the effects of $y_{ij}$ values that are extremely large by taking the square-root of species profiles containing relative abundances. 
...
Partitions the variation of response variables among 2, 3, or more groups of explanatory variables.
* _e.g._ large-scale and small-scale
* _e.g._ abiotic and biotic
* _e.g._ phylogenetic, functional and taxonomic
...
.center[![:scale 55%](images/VarPart.png)]
...
.small[.alert[Note]: Make sure you have loaded the `vegan` package!
...
---
# Variation partitioning: _what is happening?_
...
<br>
```{r, echo=FALSE}
showvarparts(2, bg=2:3)
```
]

???

√ indicates the 3 regression o r canonical analyses that have t o b e computed.
...
The LDA divides your response variable into groups according to a factor by finding combination of the variables that give **best possible separation between groups**.

Linear discriminant analysis is not just a form to reduce dimensionality, but it is a robust classification procedure.
...
In a nutshell, the approach of LDA is similar to Principal Component Analysis. 

In addition to finding the principal components that maximize the variance of our data, we also interested in axes that maximize separation between pre-defined classes.

.pull-left[

LDA can be derived from simple probabilistic predicting the _class_ conditional distribution of the data $P(X|y=k)$ for each class $k$.  

$k$ is priory selected to maximize this conditional probability. 
]

.pull-right[
```{r eval=T, echo=FALSE, message=FALSE, warning=FALSE, fig.dim = c(5,5)}
library(scales)
...
ggplot(datPred, aes(x=LD1, y=LD2) ) +
  ...
  geom_point(data = datPred, size = 3, aes(pch = Species,  colour=Species)) +
  scale_fill_manual(values=colorslight,guide=F) +
  scale_x_continuous(limits=range(datPred$LD1), expand=c(0,0)) +
  scale_y_continuous(limits=range(datPred$LD2), expand=c(0,0))+
...
P(X|y) is usually modelled as p multivariate Gaussian distribution with known density.

LDA looks for linear combinations of variables that best explain the data
...
P(X|y=k)   for each class k.  k is selected such that it maximize this conditional probability. P(X|y) is usually modeled as p multivariate Gaussian distribution with known density.
...
We know that environmental variables generally change with latitude. This might lead us to the following question:
...
.comment[We can try discriminating groups with LDA to answer this question!]
...
Let us begin by loading geographical coordinates for the Doubs River sites:
```{r}
# Load geographical coordinates for the Doubs sites
spa <- read.csv("data/doubsspa.csv", row.names = 1)
spa$site <- 1:nrow(spa) # add site numbers

spa <- spa[-8, ] # remove site 8, which lacks species
```

--

<br>
We can then assign sites into three groups based on latitudinal bands they occupy:
```{r}
...
```{r, fig.width = 7.5, fig.height = 6, echo = T, eval = F}
ggplot(data = spa) +
  geom_point(
    aes(x = x, 
        y = y, 
...
# LDA in `R`: Assumption checking

.alert[Note]: One needs to assess whether the multivariate homogeneity of within-group variances is being violated before proceeding (see Borcard et al. 2011).

.pull-left[
You can do this using the `vegan::betadisper()` function.

```{r}
(env.d.betadisper <- betadisper(
  dist(env), 
  spa$group))
```
]
...
.pull-right[
Then, test for within-group diferences in variance:

```{r}
anova(env.d.betadisper)
```
```{r}
permutest(env.d.betadisper)
```
]
...
For the purposes of this workshop, we will move straight into the LDA:

```{r}
library(MASS)

# Run the LDA grouping sites into 
# latitude groups based on env data
...
lda.plotdf <- data.frame(group = spa$group, 
                         lda = predict(LDA)$x)
```
]
...
diag(proportions(spe.table, margin = 1))
```
] 

All sites were _correctly_ classified (*Proportion of corrected classification = 1*) into the latitude groups based on environmental variables.
...
Let us **predict the grouping** of five new sites using our LDA results:
.small[
```{r}
# Load the new site data
classify.me <- read.csv("data/classifyme.csv", 
                        header = TRUE)

# remove das
classify.me <- subset(classify.me, 
                      select = -das)

# Predict grouping of new sites
predict.group <- predict(LDA, 
                         newdata = classify.me)
